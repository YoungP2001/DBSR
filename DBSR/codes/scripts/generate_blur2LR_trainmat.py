...
def generate_mod_LR_bic():
    ...
    savedir = "/home/lwd/Dataset_yp/HSISR/Chikusei/8/Chikusei_x8_blur2/trains_large"
    ...
    for file in files:
        ...
        #scipy.io.savemat(os.path.join(savedir, 'blur_{}'.format(file)),
                         #{'hr': label, 'lr_blur': image_LR_blur,'lr_blured_t':lr_blured_t,'lr_t':lr_t,'input':input,'ker_map':ker_map,'kernels':kernels})
        scipy.io.savemat(os.path.join(savedir, 'blur_{}'.format(file)),
                         {'hr': label, 'lr_blur': image_LR_blur,'lr_blured_t':lr_blured_t,'lr_t':lr_t})

        #data = loadmat(os.path.join(folder, filename))

        # # save images of blurred LR
