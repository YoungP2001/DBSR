...
import codes.config.DBSR.models.lr_scheduler as lr_scheduler
import codes.config.DBSR.models.networks as networks
from codes.config.DBSR.models.modules.loss import CharbonnierLoss, CorrectionLoss,myLoss

from .base_model import BaseModel
# import sys
# sys.path.append("/data/yp/DBSR-SR-master")  # 添加路径一定要添加到上一级目录，这样才能引用下面的目录！！！
# from codes.utils import BatchBlur
...
class B_Model(BaseModel):
    def __init__(self, opt):
        ...
        if self.is_train:
            ...
            if loss_type == "l1":
                ...
                self.cri_pix = myLoss().to(self.device)
            else:
    ...
    def feed_data(self, LR_img, GT_img=None, kernel=None, ker_map=None, lr_blured=None, lr=None):
        self.var_L = LR_img.to(self.device) #模糊，加噪的LR
        ...
        if not (lr_blured is None):
            self.lr_blured = lr_blured.to(self.device) #对模糊后HR的下采样 =LR_img(无加噪）
        if not (lr is None):
            self.lr = lr.to(self.device) #对没有模糊HR的下采样 bicubic


    def optimize_parameters(self, step):
        sr, kernel = self.netG(self.var_L) # 将LR img喂给网络
        ...
        print("total_loss:", total_loss.item(),"step:",step)
...
# 通过对输入图像进行不同的转换（水平翻转、垂直翻转、转置）得到多个输入图像，对每个输入图像进行超分辨率图像的生成，
# 最后将生成的所有超分辨率图像进行反转变换并拼接，得到最终的超分辨率图像。
    def test_x8(self):
    ...
    def get_current_visuals(self):
        ...
        out_dict["Batch_GT"] = self.real_H.detach().float().cpu()
        return out_dict
